...
function BagelCard({ item }: { item: FlavorItem }) {
...
function CreamCheeseCard({ item }: { item: FlavorItem }) {
  const [err, setErr] = useState(false);
  return (
    <div className="group flex flex-col bg-white rounded-xl overflow-hidden border border-rose-100 shadow-sm card-hover cursor-default">
      <div className="relative h-32 overflow-hidden bg-rose-50">
        {item.image && !err ? (
          <img
            src={img(item.image)}
            alt={item.name}
...
export default function BagelFlavorGrid({
  ...
  bagelFlavors: FlavorItem[];
  creamCheeseFlavors: FlavorItem[];
}) {
  return (
    <>
      {/* Bagel flavors */}
      ...
      {creamCheeseFlavors.length > 0 && (
        <div className="mt-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {creamCheeseFlavors.map((item) => (
              <CreamCheeseCard key={item.id} item={item} />
            ))}
          </div>
        </div>
      )}
